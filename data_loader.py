...
class MMDataset(Dataset):
    ...
    def __init_mosi(self):
        ...
        if self.args.get('use_bert', None):
            self.text = data[self.mode]['text_bert'].astype(np.float32) # (1284, 3, 50)
            self.args['feature_dims'][0] = 768
        else:
            self.text = data[self.mode]['text'].astype(np.float32) # (1284, 50, 768)
        ...
        if self.args.get('data_dropping'):
            self.text_d, self.text_length, self.text_mask, self.text_dropping_mask = self.generate_d(self.text[:,0,:], self.text[:,1,:], None,
                                                                                        self.args.dropping_rate[0], self.args.dropping_seed[0], mode='text')
            
            Input_ids_d = np.expand_dims(self.text_d, 1)
            Input_mask = np.expand_dims(self.text_mask, 1)
            Segment_ids = np.expand_dims(self.text[:,2,:], 1)
            self.text_d = np.concatenate((Input_ids_d, Input_mask, Segment_ids), axis=1)

            if self.args['need_data_aligned']:
                self.audio_lengths = np.sum(self.text[:,1,:], axis=1, dtype=np.int32)
                self.vision_lengths = np.sum(self.text[:,1,:], axis=1, dtype=np.int32)

            self.audio_d, self.audio_length, self.audio_mask, self.audio_dropping_mask = self.generate_d(self.audio, None, self.audio_lengths,
                                                                                        self.args.dropping_rate[1], self.args.dropping_seed[1], mode='audio')
            self.vision_d, self.vision_length, self.vision_mask, self.vision_dropping_mask = self.generate_d(self.vision, None, self.vision_lengths,
                                                                                        self.args.dropping_rate[2], self.args.dropping_seed[2], mode='vision')
    ...
    def generate_d(self, modality, input_mask, input_len, dropping_rate, dropping_seed, mode='text'):
        # NOTE: dropping some samples entirely, input shape: (num_examples, max_len, feature_dim)
        if mode == 'text':
        ...
        dropping_mask = (np.random.uniform(size=input_mask.shape[0]) > dropping_rate).repeat(input_mask.shape[1]).reshape(input_mask.shape)
        dropping_mask = dropping_mask * input_mask
        ...
        if mode == 'text':
            ...
            for i, instance in enumerate(dropping_mask):
                instance[0] = instance[input_len[i] - 1] = 1
            
            modality_d = dropping_mask * modality + (100 * np.ones_like(modality)) * (input_mask - dropping_mask) # UNK token: 100.
        elif mode == 'audio' or mode == 'vision':
            modality_d = dropping_mask.reshape(modality.shape[0], modality.shape[1], 1) * modality
        
        return modality_d, input_len, input_mask, dropping_mask
        
        

    def generate_m(self, modality, input_mask, input_len, missing_rate, missing_seed, mode='text'):
        # When Text Modality is not Encoded By Bert: Generate Missing at Token Level
        # if self.args.get('use_bert', None):
        if mode == 'text':
            input_len = np.argmin(input_mask, axis=1)
        elif mode == 'audio' or mode == 'vision':
    ...
    def __getitem__(self, index):
        ...
        if self.args.get('data_dropping'):
            sample['text_d'] = torch.Tensor(self.text_d[index])
            sample['text_dropping_mask'] = torch.Tensor(self.text_dropping_mask[index])
            sample['audio_d'] = torch.Tensor(self.audio_d[index])
            sample['audio_lengths'] = self.audio_lengths[index]
            sample['audio_mask'] = self.audio_mask[index]
            sample['audio_dropping_mask'] = torch.Tensor(self.audio_dropping_mask[index])
            sample['vision_d'] = torch.Tensor(self.vision_d[index])
            sample['vision_lengths'] = self.vision_lengths[index]
            sample['vision_mask'] = self.vision_mask[index]
            sample['vision_dropping_mask'] = torch.Tensor(self.vision_dropping_mask[index])
    ...
    def _retrieve_top_K_samples(self, query: list[np.ndarray], db: list[np.ndarray], top_k: int, metric: str = 'cosine'):
        """
        Retrieve top K samples from db w.r.t. query.

        :param query: List of numpy arrays representing the query samples.
        :param db: List of numpy arrays representing the database samples.
        :param top_k: Number of top samples to retrieve.
        :param metric: Similarity metric to use ('cosine' or 'euclidean').
        :return: Indices of the top K samples in the database.
        """
        pass
