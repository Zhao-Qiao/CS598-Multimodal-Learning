...
from .config import get_config_regression, get_config_tune, get_confit_exp
...
def MMSA_run(
    ...
    if config_file is not None:
        config_file = Path(__file__).parent / "config" / config_file
    ...
    if exp_config is not None and exp_task is not None:
        exp_config = get_confit_exp(exp_config, exp_task)
    ...
    experiment_settings = exp_config[variant]
    ...
    for exp in experiment_settings:
        ...
        d_exp_config = exp_config
        d_exp_config.update({variant: exp})
        logger.info(f"Current experiment settings: [{variant}: {exp}]")
        ...
        if is_tune: # run tune
            ...
            # merge exp config
            initial_args.update(exp_config)
            ...
            print("Running on device: ", initial_args['device'])
