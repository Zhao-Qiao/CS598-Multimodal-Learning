...
def get_confit_exp(config_file: str, task: str) -> dict:
    """
    Get the config of given dataset and model from config file.
    ...
    assert task in config_all, f"Task {task} not found in config file"
        
    task_args = config_all[task]
    config = {}
    config.update(task_args)
